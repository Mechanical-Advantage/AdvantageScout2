<script>
    ...
    let enabled = ["Cameron Earle", "A Very-Long-Name for Testing Purposes", "", "1", "Minecraft Steve", "FRC Team 2713 - The iGoblins", "Gryffin"]
    function testFunc() {
        alert("Hello World");
    }
    ...
    function mouseCoordHandler(e) {
        ...
        $withinBox = dropBoxRect !== null && e.clientX > dropBoxRect.x && e.clientX < dropBoxRect.x + dropBoxRect.width && e.clientY > dropBoxRect.y && e.clientY < dropBoxRect.y + dropBoxRect.height;
        $withinTrash = trashRect !== null && e.clientX > trashRect.x && e.clientX < trashRect.x + trashRect.width && e.clientY > trashRect.y && e.clientY < trashRect.y + trashRect.height;
    }
...
<div class="z-20 w-fit h-fit" on:mousemove={mouseCoordHandler} on:mouseup={mouseUp}>
    ...
    <svg id="trash" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke={(($withinTrash && $movin) ? 'red' : 'white')} class="{(($withinTrash && $movin) ? 'w-32 h-32 ml-[615px] mt-[110px]' : 'w-24 h-24 ml-[630px] mt-[125px]')} fixed ">
        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
    </svg>

    <div id="dropBox" style="width:600px;height:{dropBoxHeight}px;border:4px solid {((!($movin&&$withinBox&&!$activeState)) ? '#FFFFFF' : '#00FF00')};" class="relative mt-6 ml-3">
        <div class="fixed z-20 w-fit h-fit" on:mousemove={mouseCoordHandler}>
            <div class="grid  grid-cols-4 grid-rows-none gap-32 w-1/3 h-fit mt-10 ml-[55px] z-30 relative overflow-visible">
    ...
    <form on:submit={addName} action="#">
        ...
        <label for="entry-modal" class="modal cursor-pointer">
            <label class="modal-box relative" for="entry-modal">
                <input type="text" placeholder="Scout Name" class="input input-bordered w-full max-w-xs" bind:value={addScoutEntry} />
